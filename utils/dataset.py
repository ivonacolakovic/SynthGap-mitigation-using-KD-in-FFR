...
class MXSyntheticFaceDataset(Dataset):
    def __init__(self, root_dir, local_rank,from_file = "",is_train=True,transform=None):
    ...
    def __getitem__(self, index):
        ...
        sample = cv2.imread(image_path)
        sample = cv2.cvtColor(sample,cv2.COLOR_BGR2RGB)
        #extra_img_path = image_path.replace("ca-cpd25-synthetic-uniform-10050/ca-cpd25-synthetic-uniform-10050","")
        #extra_img_path = image_path.replace("ca-cpd25-synthetic-uniform-10050/ca-cpd25-synthetic-uniform-10050","")
        #extra_img_path = image_path.replace("ca-cpd25-synthetic-uniform-10050/ca-cpd25-synthetic-uniform-10050","")
        #extra_img_path = image_path.replace(self.root_dir,"").replace(".jpg","")
        extra_img_path = "/".join(self.image_paths[index].replace(".jpg","").replace(".png","").split('/')[-3:])
        if self.transform is not None:
            sample = self.transform(sample)
